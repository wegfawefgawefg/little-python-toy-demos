...
# --- New Entity Generation Functions ---


def generate_flower_patch(x, z, ground_h):
    """Generate a flower patch entity at (x,z) just above ground.
    The flower color is chosen randomly from yellow, red, or pink."""
    flower_colors = [(255, 255, 0), (255, 0, 0), (255, 192, 203)]
    color = random.choice(flower_colors)
    pos = (x + 0.5, ground_h + 1, z + 0.5)
    return {"type": "flower", "pos": pos, "color": color}
...
def generate_bunny(cx, cz, chunk):
    ...
    return {"type": "bunny", "pos": pos, "hop_timer": hop_timer, "hop_cycle": hop_cycle}
...
def generate_chunk(cx, cz):
    ...
    for lx in range(CHUNK_SIZE):
        for lz in range(CHUNK_SIZE):
            ...
            # Water if ground is below water level.
            if h < water_level:
            ...
            if h >= water_level and random.random() < 0.05:
    ...
    # Bunny generation: low chance per chunk.
...
while True:
            ...
            if key in chunk_entities:
                for entity in chunk_entities[key]:
                    if entity["type"] == "grass":
                        ...
                        depth = cam_space[2]
                        combined_draw_list.append(
                            (depth, "flower", proj, factor, entity["color"])
                        )
                        ...
                        if proj is None:
                            continue
                        depth = cam_space[2]
                        # Bunny hop animation: three frames.
                        t = entity["hop_timer"]
                        cycle = entity["hop_cycle"]
                        if t < cycle / 3:
                            offset_y = 0
                        elif t < 2 * cycle / 3:
                            offset_y = -2 * factor  # bunny going up
                        else:
                            offset_y = 2 * factor  # bunny coming down
                        combined_draw_list.append(
                            (depth, "bunny", proj, factor, offset_y)
                        )
    ...
    for item in combined_draw_list:
        if item[1] == "block":
            ...
            mod_color = scale_color(color, brightness)
            pygame.draw.circle(
                render_surf, mod_color, (int(sx), int(sy)), max(1, int(2 * factor))
            )
        elif item[1] == "bunny":
            depth, _, proj, factor, offset_y = item
            sx, sy = proj
            brightness = max(0, 1 - (depth / BLOCK_MAX_DIST) ** 2)
            bunny_color = scale_color((255, 255, 255), brightness)
            pygame.draw.circle(
                render_surf,
                bunny_color,
                (int(sx), int(sy + offset_y)),
                max(1, int(3 * factor)),
            )
